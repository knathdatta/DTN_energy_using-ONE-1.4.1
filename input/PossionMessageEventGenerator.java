 ...
 * be configured.
 */
public class PossionMessageEventGenerator implements EventQueue {
	 ...
	 */
	public PossionMessageEventGenerator(Settings s){
		...
		this.idPrefix = s.getSetting(MESSAGE_ID_PREFIX_S);
                ...
                if (s.contains(MESSAGE_RATE)) {
			this.rate = this.rate = s.getDouble(MESSAGE_RATE);
		}
		else {
			this.rate = 0;
		}
		...
		this.nextEventsTime = possionMessageInterval(rate);//(this.msgTime != null ? this.msgTime[0] : 0) + msgInterval[0] + (msgInterval[0] == msgInterval[1] ? 0 : 	rng.nextInt(msgInterval[1] - msgInterval[0]));
	}
	...
	public ExternalEvent nextEvent() {
		...
		MessageCreateEvent mce = new MessageCreateEvent(from, to, this.getID(), msgSize, responseSize, this.nextEventsTime);
                ...
                this.nextEventsTime += possionMessageInterval(rate);
	...
	protected String getID(){
        ...
        protected double possionMessageInterval(double rate)
        {
            Random r=new Random();
            
            double nextTime = -Math.log(r.nextInt(RAND_MAX)/(RAND_MAX+1.0))*rate;
            if(hostRange[0]==23)
